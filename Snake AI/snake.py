...
class SnakeGame:
    ...
    def play_step(self):
        ...
        game_over = False
        # Ако се е докоснала змията до себе си или границите на полетоF
    ...
    def _move(self, direction):
        x = self.head.x
        y = self.head.y
        # Променяме координатите на главата в зависимост от посоката
    ...
    # Проверяваме дали змията е достигнала границите на полето и я преместваме на противоположната страна, ако е необходимо
        if x >= self.w:
            x = 0
        elif x < 0:
            x = self.w - BLOCK_SIZE
